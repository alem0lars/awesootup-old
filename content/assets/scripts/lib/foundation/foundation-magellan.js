...
(function ($, window, undefined) {
  ...
  $.fn.foundationMagellan = function (options) {
    var $window = $(window),
        ...
        defaults = {
          threshold: ($fixedMagellan.length) ? $fixedMagellan.outerHeight(true)
              : 0,
          activeClass: 'active'
        },
    ...
    $expedition.find('[data-magellan-arrival]:first')
        .addClass($expedition.attr('data-magellan-active-class')
            || options.activeClass);
    ...
    $window.on('scroll.magellan', function () {
      ...
      $fixedMagellan.each(function () {
        ...
        var fixed_position = (windowScrollTop + options.threshold)
            > $expedition.data("magellan-top-offset");
    ...
    if ($lastDestination.length > 0) {
      $window.on('scroll.magellan', function (e) {
        ...
        $('[data-magellan-destination]').each(function () {
          ...
          if (scrolltopPlusHeight >= $document.outerHeight(true)
              && lastDestinationTop > windowScrollTop && lastDestinationTop
              < scrolltopPlusHeight) {
